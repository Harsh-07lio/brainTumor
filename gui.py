...
#         WindowLabel = tkinter.Label(self.FirstFrame.getFrames(), text="Developed by Harsh ujjainiya", height=2, width=40)
...
#         backBtn = tkinter.Button(self.FirstFrame.getFrames(), text="Back", width=8, command=self.goBackToMainPage)
#         backBtn.place(x=700, y=602) 
#         MainWindow.mainloop()

#     def getListOfWinFrame(self):
#         return self.listOfWinFrame
#     def goBackToMainPage(self):
#         for frame in self.listOfWinFrame:
#             frame.hide()
#         self.FirstFrame.unhide()
...
class Gui:
    ...
    def __init__(self):
        ...
        RB1 = tkinter.Radiobutton(self.FirstFrame.getFrames(), text="Detect Tumor", variable=self.val,
                                  value=1, command=self.check)
        RB1.place(x=200, y=200)
        RB2 = tkinter.Radiobutton(self.FirstFrame.getFrames(), text="View Tumor Region", variable=self.val,
                                  value=2, command=self.check)
        RB2.place(x=200, y=250)
    ...
    def check(self):
        ...
        if self.val.get() == 1:
            ...
            # Create BACK button on second frame also
            secFrameBackBtn = tkinter.Button(secFrame.getFrames(), text="Back", width=8, command=self.goBackToMainPage)
            secFrameBackBtn.place(x=700, y=602)
